function [Gray_Cell_Measurements,C_Gray_Paramters] = GLCM_Features4(GrayCell_Mask)
C_Gray_Paramters=[{'Autocorrelation'},{'Contrast_M'},{'Correlation_M'},{'Correlation_P'},{'Cluster SD'},{'Cluster Prominence'},{'Cluster Shade'},{'Dissimilarity'},{'Energy'}   ,...
                {'Entropy'},{'Homogeneity_M'},{'Homogeneity_P'},{'Maximum_probability'},{'SumSquaresVariance'},{'Sum_mean'},{'Sum_variance'},{'SumEntropy'},...
                {'DifferenceVariance'},{'DifferenceEntropy'},{'InformationMeasureCorrelation1'},{'InformaitonmeasureCorrelation2'},{'InverseDifferenceHomom'},...
                {'InverseDifferenceNormalized(INN)'},{'InverseDifferenceMomentNormalized'},];
            
            
%To make the parameters rotation invariant
%the parameters are calculated in 4 different directions then the average
%is reported so the offset is set to be :
Offset=[0 1; -1 1;-1 0;-1 -1]; 
pairs=0;
% Written by Avinash Uppuluri
% Last editted by Elaheh Alizadeh
glcmin=graycomatrix(GrayCell_Mask,'Offset',Offset,'NumLevels',25);
...
% Added by Elaheh  Alizadeh
% Cluster Standard deviation                (out.csd)               
...
out.cmean=zeros(1,size_glcm_3);%Added By Elaheh
out.csd = zeros(1,size_glcm_3);%Added By Elaheh
...
for k = 1:size_glcm_3 % number glcms
   ...
   out.cmean(k) = sum(sum(((i_matrix + j_matrix - u_x(k) - u_y(k))).*...
                glcm(:,:,k))); 
   out.csd(k) = sum(sum(((i_matrix + j_matrix - u_x(k) - u_y(k)).^2).*...
                glcm(:,:,k)));          
   ...
   out.senth(k) =  - sum(p_xplusy(:,k).*...
    ...
    % computational instability 
    % http://murphylab.web.cmu.edu/publications/boland/boland_node26.html    
     Measurements=cat(1,out.autoc, out.contr, out.corrm, out.corrp,out.csd,out.cprom,...
                    out.cshad, out.dissi, out.energ, out.entro, out.homom, out.homop,...
                      out.maxpr, out.sosvh, out.savgh, out.svarh, out.senth, out.dvarh,...
                      out.denth, out.inf1h, out.inf2h, out.homom, out.indnc , out.idmnc);
                  
     %Gray_Cell_Measurements=transpose(cat(1,Measurements(:,1),Measurements(:,2),Measurements(:,3),Measurements(:,4)));
     Gray_Cell_Measurements=cat(2,Measurements(:,1),Measurements(:,2),Measurements(:,3),Measurements(:,4));
     
     Gray_Cell_Measurements=transpose(mean(Gray_Cell_Measurements,2));
end
